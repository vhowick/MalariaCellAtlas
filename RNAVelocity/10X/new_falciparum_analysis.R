...
ann <- read.table("~/Collaborations/MCA/For10XVelocity/pf3d7100scmapclusts2methodwithFDR_20190108.csv", sep=",", header=TRUE)

# Read in Data

emat <- readRDS("new_pf_out_S_matrix.rds")
smat <- readRDS("new_pf_out_U_matrix.rds")

require("Matrix")

cell_names <- read.delim("new_pf_out_ca_matrix.csv", ",", header=TRUE)
cell_names <- sub("possorted_genome_bam_D8EDE:", "", cell_names$CellID)
cell_names <- sub("x$", "", cell_names)

gene_ann <- read.delim("new_pf_out_ra_matrix.csv", ",", header=TRUE)
...
# Consistent cells
emat <- emat[,match(ann[,2], colnames(emat))]
smat <- smat[,match(ann[,2], colnames(smat))]
...
saveRDS(dat, file="new_pf_10X_velodata.rds")
...
saveRDS(rvel.qf, file="new_pf_10X_veloEst.rds")
...
dev.off()
saveRDS(pca.out, file="new_pf_10X_veloPCA.rds")
