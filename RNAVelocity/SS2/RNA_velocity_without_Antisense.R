...
do_QC <- function(dat, qc_cells) {
	...
	new_names <- qc_cells[good_cells %in% colnames(dat$emat),1]
	emat <- dat$emat;
...
image_name="SPZ_RNAvelo_noAnti.png"
prefix="SPZ"
...
cell_cols <- cell_cols[match(colnames(stuff$emat), names(cell_cols))]
ref_cells <- names(cell_cols)
image_name="OOKOO_RNAvelo_noAnti.png"
prefix="OOKOO"
...
cell_cols <- cell_cols[match(colnames(stuff$emat), names(cell_cols))]
ref_cells <- names(cell_cols)
image_name="EEF_RNAvelo_noAnti.png"
prefix="EEF"
anti <- readRDS("AntiSense/EEF_smoother_Antisense.rds")
} else if (args[1]=="idc") {
### IDC
require("scater")
require("velocyto.R")
...
cell_cols <- cell_cols[match(colnames(stuff$emat), names(cell_cols))]
ref_cells <- names(cell_cols)
image_name="IDC_RNAvelo_noAnti.png"
prefix="IDC"
...
image_name="Blood_RNAvelo_noAnti.png"
prefix="Blood"
anti <- readRDS("AntiSense/Blood_smoother_Antisense.rds")
} else {
### All
qc_cells1 <- read.delim("SPZgoodcells.csv", sep=",")
qc_cells2 <- read.delim("OOKOOgoodcells.csv", sep=",")
...
image_name <- "All_RNAvelo_noAnti.png"
prefix="All"
###
}
...
set.seed(2819)
rvel.qf <- gene.relative.velocity.estimates(stuff$emat, stuff$smat, deltaT=10, kCells = 5,fit.quantile = 0.075, min.nmat.emat.slope=0.1, min.nmat.emat.correlation=0.5, kGenes=1, steady.state.cells=ref_cells, zero.offset=TRUE, smat=stuff$nmat)
saveRDS(rvel.qf, paste(prefix, "_RNAVelo_Estimates_noAnti.rds", sep=""))
...
pca.out <- pca.velocity.plot(rvel.qf,nPcs=5,plot.cols=2,cex=1.2,pcount=0.1,pc.multipliers=c(1,-1,-1,-1,-1), show.grid.flow=TRUE, grid.n=30, cell.colors=cell_cols)	
saveRDS(pca.out, paste(prefix, "_RNAVelo_PCAobj_noAnti.rds", sep=""))
dev.off()

emb <- pca.out$epc@scores[,1:2]

scores <- Matrix::rowMeans(abs(rvel.qf$deltaE))
saveRDS(sort(scores), paste(prefix, "_RNAVelo_AllRankedGenes_noAnti.rds", sep=""))
print("pca")
top_genes <- head(sort(scores, decreasing=T),20)

for (g in names(top_genes)) {

	png(paste(prefix, g, "rnavelo_fits_noAnti.png", sep="_"), width=4*4, height=4, units="in", res=300)
